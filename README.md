...
<body class="flex items-center justify-center min-h-screen text-slate-900 overflow-x-hidden">
    ...
    <div class="app-shell bg-white flex flex-col w-full min-h-screen sm:min-h-0 pb-24 transition-all-custom">
        ...
        <!-- BOTÕES DE NAVEGAÇÃO DE ABAS INFERIORES -->
    ...
    <script>
        ...
        function regexMappingParser(rawText) {
            ...
            // 1. REGEX DE CARTÃO: 4 dígitos finais precedidos de exatamente 14 asteriscos (Anexo 4)
            ...
            // 3. REGEX DO ESTABELECIMENTO: sempre junto com o número de CPF ou CNPJ (Anexo 2)
            ...
            // 4. VALOR DA NOTA: Esse dado é o número que sempre está em destaque no cupom (Anexo 3)
            // Tratamos variações comuns de OCR para o símbolo "R$" (como RS, R5, H$, K$, P$, B$)
            let textNormalizedForValue = rawText
                .replace(/[RH5KPSB]{1,2}\s*[\$S5]\s*/g, 'R$ ')
                .replace(/R\s+[\$S5]/g, 'R$');

            // Regex de alta sensibilidade para encontrar valores monetários após o símbolo "R$"
            // Aceita quebras de linha e espaços múltiplos entre o símbolo e o número
            const highConfidenceValueRegex = /R\$\s*(\d{1,3}(?:\.\d{3})*,\d{2}|\d+[\.,]\d{2})/gi;
            let rsValues = [];
            let rsMatch;
            
            while ((rsMatch = highConfidenceValueRegex.exec(textNormalizedForValue)) !== null) {
            ...
            if (rsValues.length > 0) {
                // O valor em destaque do cupom é prioritariamente o primeiro/principal associado ao R$
                valor = rsValues[0];
            } else {
                // Fallback inteligente: Busca qualquer padrão numérico de moeda (ex: "10,00" ou "1.250,50")
                // que não esteja associado a datas ou CPFs/CNPJs e prioriza o de maior destaque
                const genericPriceRegex = /\b(\d{1,3}(?:\.\d{3})*,\d{2}|\d+[\.,]\d{2})\b/g;
                let candidateValues = [];
                let genericMatch;
                
                while ((genericMatch = genericPriceRegex.exec(rawText)) !== null) {
                    const val = parseNumber(genericMatch[1]);
                    // Filtra para evitar capturar frações de CPF/CNPJ ou datas
                    const contextStart = Math.max(0, genericMatch.index - 8);
                    const contextEnd = Math.min(rawText.length, genericMatch.index + genericMatch[1].length + 8);
                    const contextStr = rawText.substring(contextStart, contextEnd);
                    
                    const looksLikeCpfOrDate = /[\/\.-]\d{2,4}/.test(contextStr) || /\d{3,}[\.-]/.test(contextStr);
                    
                    if (val > 0 && val < 99999 && !looksLikeCpfOrDate) {
                        candidateValues.push(val);
                    }
                }

                if (candidateValues.length > 0) {
                    valor = candidateValues[0];
                }
            }
        ...
        async function handleFileUpload(event) {
            ...
            try {
                ...
                if (parsedData && (parsedData.valor > 0 || parsedData.estabelecimento !== "Estabelecimento Desconhecido")) {
                    ...
                    showToast("OCR completo, mas sem dados suficientes estruturados.");
                }
            } catch (err) {
                console.error(err);
