...
<body class="flex items-center justify-center min-h-screen text-slate-900 overflow-x-hidden">
    ...
    <script>
        ...
        function regexMappingParser(rawText) {
            ...
            // 1. REGEX DE CARTÃO (Final com 4 dígitos)
            ...
            while ((rsMatch = rsRegex.exec(rawText)) !== null) {
                ...
                if (val > 0 && val < 9999) rsValues.push(val);
            }
        ...
        async function handleFileUpload(event) {
            ...
            try {
                ...
                if (parsedData && parsedData.valor > 0) {
                    addPendingItem(parsedData);
                } else {
                    showToast("OCR completo, mas sem valores detectados.");
                }
            } catch (err) {
                console.error(err);
